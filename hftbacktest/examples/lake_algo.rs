    ...
    PA: PriceAction,
{
    ...
    let mut acc_tick_open_time = 0i64;
    let mut acc_tick_qty = 0.0;
    ...
    while hbt.elapse(100_000_000).unwrap() {
        ...
        let (kmaps,last_open_time) = price_action.kmaps(5 * 60 * 1_000_000_000, 48);
        if kmaps.len() < 3 {
            continue;
        }
        let open_time = nanos_to_ymdhms(last_open_time );
        let mut last_height = kmaps.get(&(last_open_time - 5 * 60 * 1_000_000_000)).unwrap().high_tick;
        let mut last_low = kmaps.get(&(last_open_time - 5 * 60 * 1_000_000_000)).unwrap().low_tick;
        let findlen = if kmaps.len() > 5 {5} else {kmaps.len()};
        ...
        if position == 0.0 {
            // long
            if below_buys >= 5 && best_ask_tick >= acc_buy_tick && !orders.contains_key(&((best_ask / tick_size) as u64)){
                stop_loss = (acc_buy_tick - 100) as f64 * tick_size;
                stop_profit = stop_loss + 600.0 * tick_size;
                ...
                println!("Long-open::{}:best_bid:{},best_ask:{},stop_loss:{},stop_profit:{},acc_sell_tick:{},above_sells:{}",open_time,best_bid,best_ask,stop_loss,stop_profit,acc_buy_tick,below_buys);
                acc_sell_tick = 0;
                above_sells = 0;
                acc_buy_tick = 0;
                below_buys = 0;
            ...
            // short
            if above_sells >= 5 && best_bid_tick <= acc_sell_tick && !orders.contains_key(&((best_bid / tick_size) as u64)){
                stop_loss = (acc_sell_tick + 100) as f64 * tick_size;
                stop_profit = stop_loss - 600.0 * tick_size;
                ...
                // acc_buy_tick = 0;
                // below_buys = 0;
                println!("Short-open::{}:best_bid:{},best_ask:{},stop_loss:{},stop_profit:{},acc_buy_tick:{},below_buys:{}",open_time,best_bid,best_ask,stop_loss,stop_profit,acc_sell_tick,above_sells);
                acc_sell_tick = 0;
                above_sells = 0;
                acc_buy_tick = 0;
                below_buys = 0;
            ...
            if position > 0.0 {
                ...
                if acc_sell_tick > 0 && above_sells > 5 {
                    stop_loss = (acc_sell_tick + 100) as f64 * tick_size;
                    stop_profit = stop_loss - 800.0 * tick_size;
